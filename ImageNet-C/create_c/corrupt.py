...
def plasma_fractal(mapsize=IMG_SIZE, wibbledecay=3):
    """
...
def glass_blur(x, severity=1):
    ...
    x = np.uint8(gaussian(np.array(x) / 255., sigma=c[0], channel_axis=None) * 255)
    ...
    return np.clip(gaussian(x / 255., sigma=c[0], channel_axis=None), 0, 1) * 255
...
def fog(x, severity=1):
    # not usable corruption, plasma_fractal function throws lots of errors
...
def frost(x, severity=1):
    ...
    # randomly crop and convert to rgbs
    x_start, y_start = np.random.randint(0, abs(frost.shape[0] - IMG_SIZE)), np.random.randint(0, abs(frost.shape[1] - IMG_SIZE))
    frost = frost[x_start:x_start + IMG_SIZE, y_start:y_start + IMG_SIZE][..., [2, 1, 0]]
    np_x = np.array(x)
    frost = cv2.resize(frost, (np_x.shape[1], np_x.shape[0]))

    assert frost.shape == np_x.shape, f"Shapes do not match: {frost.shape} vs {x.shape}"

    return np.clip(c[0] * np_x + c[1] * frost, 0, 255)
